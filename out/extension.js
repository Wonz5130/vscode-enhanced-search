...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('enhanced-search-view', provider));
    // 注册命令
    let openSearchPanelCommand = vscode.commands.registerCommand('enhanced-search.openSearchPanel', () => {
        searchPanel_1.SearchPanel.createOrShow(context.extensionUri);
    });
    ...
    let searchClass = vscode.commands.registerCommand('enhanced-search.searchClass', () => {
        const panel = searchPanel_1.SearchPanel.createOrShow(context.extensionUri);
    ...
    context.subscriptions.push(searchClass, searchMethod, searchFile, searchGlobalVar);
    // 注册侧边栏视图容器激活事件
